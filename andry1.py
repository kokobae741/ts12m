...
def log(value, status='INFO KAWAN', color='[R1]'):
    value = colors('{color}''[P1]''[{time}] [P1]:: {color}{status} [P1]:: {color}{value}[CC]'.format(
...
def log_replace(value, status='INFO KAWAN', color='[G1]'):
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('SING SAAAABAR YA WA'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('OKE WA LANJUT ''[R2]'.format(self.proxy_host, self.proxy_port), color='[R1]')
        except OSError:
...
def main():
    print(colors('\n'.join([
        '',
        '               [CC]''[G1][!]TELE KAMPRETOS'
        '[G1] KAMPRET[!]',
        '',
        '[CC]'
        '[R1]BANG KOKO', '[CC]'
        '[P1]RAGABLEG KUOTA LAN PULSA', '[CC]'
        '[Y1]SUNGGUH  T E R L A L U ', '[CC]'
        '[G1]BANG KOKO',
